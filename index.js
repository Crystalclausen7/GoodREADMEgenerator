...
.prompt(questionsList).then(result => {
    ...
    axios.get(URL, { auth: { username: "", password: }}).then(res => {
        const repoNameList = [];
        ...
        axios.get(URL2, { auth: { username:'', password:}})
        .then(response => {

            inquirer
            .prompt(questionsList2)
            ...
            .then(result => {
                ...
                if (email == null) {
                    email = "User Email information is not public"
                }
...
const createMarkdown = (Info) => {
   ...
   array.push("## Questions:")
   array.push("if yoy have questions please contact " + Info.emailInfo)
